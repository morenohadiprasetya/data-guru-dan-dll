// Updated Level CRUD Data: siswa, guru, karyawan
// (full component to replace your current file)
...
export default function LevelCRUDData() {
  ...
  return (
    <div className="p-6 ml-55 min-h-screen bg-gray-50">
      <h2 className="text-3xl font-bold text-blue-700 mb-6">CRUD Level Data: siswa, guru, karyawan</h2>

      <div className="bg-white shadow-lg rounded-xl p-5 border mb-6">
        <h3 className="text-xl font-semibold text-blue-600 mb-4">{id ? "Edit Level" : "Tambah Level"}</h3>
        ...
        <button onClick={save} className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded shadow">Simpan</button>

        {id && (
          <button onClick={() => navigate("/kategori-data")} className="ml-3 px-6 py-2 bg-gray-400 text-white rounded shadow">Batal</button>
        )}
      </div>
      ...
      <div className="bg-white shadow-lg rounded-xl border overflow-hidden">
        {loading ? (
          ...
          <div className="p-5 text-center text-gray-500">Tidak ada data.</div>
        ) : (
                ...
                <tr key={d.id} className="border-b hover:bg-blue-50">
                  ...
                  <td className="p-3">
                    <div className="flex justify-center gap-3">
                      <button onClick={() => navigate(`/kategori-data/${d.id}`)} className="px-3 py-1 bg-yellow-400 hover:bg-yellow-500 text-white rounded">Edit</button>
                      <button onClick={() => remove(d.id)} className="px-3 py-1 bg-red-600 hover:bg-red-700 text-white rounded">Hapus</button>
                    </div>
                  </td>
                </tr>
              ))}
